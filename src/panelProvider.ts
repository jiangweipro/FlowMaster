...
import { StateReader, DemandSummary } from './stateReader';
import { StateWriter, ReviewGateResult } from './stateWriter';
import { TerminalManager, PhaseCompleteEvent } from './terminalManager';
import {
  PHASE_ORDER,
  PHASE_LABELS,
  PHASE_STATUS,
...
export class FlowMasterDashboardProvider {
  private panel: vscode.WebviewPanel | undefined;
  private selectedDemandId: string | null = null;
  private pendingDemandSelection: string | null = null;
  ...
  createOrShow(demandId?: string): void {
    if (demandId) {
  ...
  private sendSelectedDemand(): void {
    if (!this.panel) return;
    try {
  ...
  private handleMessage(msg: any): void {
    ...
    switch (msg.command) {
      ...
      case 'refreshState':
        ...
        break;
      case 'runPhase':
        this.terminalManager.runPhase(
          msg.demandId || msg.payload?.demandId,
          msg.phase || msg.payload?.phase,
        );
        break;
      case 'openFile':
  ...
  private handleReviewGate(
    ...
    if (result.success) {
      ...
      vscode.window.showErrorMessage(
        `[FlowMaster] ${result.message}: ${result.error}`,
      );
    }
  }
  ...
  private openFile(filePath: string): void {
    ...
    vscode.workspace.openTextDocument(absPath).then(
      ...
      (err) =>
        vscode.window.showErrorMessage(`[FlowMaster] 打开失败: ${String(err)}`),
    );
  }
