...
exports.FlowMasterDashboardProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
// ============================================================
// Dashboard WebView Panel Provider
// ============================================================
class FlowMasterDashboardProvider {
    constructor(stateReader, stateWriter, terminalManager, context) {
        this.stateReader = stateReader;
        this.stateWriter = stateWriter;
        this.terminalManager = terminalManager;
        this.context = context;
        this.selectedDemandId = null;
        this.pendingDemandSelection = null;
        this.extVersion = '';
        try {
            this.extVersion = require('../package.json').version;
        }
        catch {
    ...
    // Panel lifecycle
    // ----------------------------------------------------------
    createOrShow(demandId) {
        if (demandId) {
            this.pendingDemandSelection = demandId;
        }
        if (this.panel) {
            this.panel.reveal();
            this.sendSelectedDemand();
            return;
        }
        this.panel = vscode.window.createWebviewPanel('flowmasterDashboard', 'FlowMaster 控制台', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        this.panel.webview.onDidReceiveMessage((msg) => this.handleMessage(msg), undefined, this.context.subscriptions);
    ...
    refresh() {
        this.stateReader.invalidateCache();
        this.sendSelectedDemand();
    }
    dispose() {
        this.panel?.dispose();
    ...
    // ----------------------------------------------------------
    // Phase completion handler (called by TerminalManager)
    // ----------------------------------------------------------
    ...
    sendSelectedDemand() {
        if (!this.panel)
            return;
        try {
            const all = this.stateReader.readAllStates();
            if (all.length === 0) {
                this.panel.webview.postMessage({
                    command: 'stateUpdated',
                    payload: { demand: null, noDemands: true },
                });
                return;
            }
            // If a pending selection exists, prefer it
            if (this.pendingDemandSelection &&
                all.find((d) => d.id === this.pendingDemandSelection)) {
                this.selectedDemandId = this.pendingDemandSelection;
                this.pendingDemandSelection = null;
            }
            // If no demand selected, pick the first
            if (!this.selectedDemandId ||
                !all.find((d) => d.id === this.selectedDemandId)) {
                this.selectedDemandId = all[0].id;
            }
            const demand = all.find((d) => d.id === this.selectedDemandId);
            this.panel.webview.postMessage({
                command: 'stateUpdated',
                payload: { demand: demand || all[0], noDemands: false },
            });
        }
        catch (e) {
            console.error('[FlowMaster] sendSelectedDemand error:', e);
            this.panel.webview.postMessage({
                command: 'stateUpdated',
                payload: { demand: null, noDemands: true, error: String(e) },
            });
        }
    }
    // ----------------------------------------------------------
    ...
    handleMessage(msg) {
        ...
        switch (msg.command) {
            case 'ready':
                // Legacy compatibility — state is sent immediately on panel creation
                break;
            case 'refreshState':
                this.sendSelectedDemand();
                break;
            case 'runPhase':
                this.terminalManager.runPhase(msg.demandId || msg.payload?.demandId, msg.phase || msg.payload?.phase);
                break;
            case 'openFile':
                this.openFile(msg.path || msg.payload?.path);
                break;
            case 'reviewGate':
                this.handleReviewGate(msg.demandId || msg.payload?.demandId, msg.phase || msg.payload?.phase, msg.action || msg.payload?.action);
                break;
        }
    }
    handleReviewGate(demandId, phase, action) {
        if (action !== 'pass' && action !== 'reject') {
            vscode.window.showErrorMessage(`[FlowMaster] 无效的审核操作: ${action}`);
            return;
        }
        const result = this.stateWriter.reviewGate(demandId, phase, action);
        if (result.success) {
            vscode.window.showInformationMessage(`[FlowMaster] ${result.message}`);
            this.stateReader.invalidateCache();
            this.refresh();
        }
        else {
            vscode.window.showErrorMessage(`[FlowMaster] ${result.message}: ${result.error}`);
        }
    }
    openFile(filePath) {
        const root = this.context.extensionPath; // fallback
        // Use workspace root for relative paths
        const folders = vscode.workspace.workspaceFolders;
        const workspaceRoot = folders && folders.length > 0 ? folders[0].uri.fsPath : process.cwd();
...
<body>
  <header id="header">
    <div style="display:flex;align-items:center;gap:8px">
      ...
      <span id="versionBadge" style="font-size:11px;color:var(--vscode-descriptionForeground);opacity:.7;margin-left:4px">v${version}</span>
    </div>
    <div style="display:flex;align-items:center;gap:8px">
...
(function(){
  ...
  const nextStepsBanner = document.getElementById('nextStepsBanner');

  const PHASE_ORDER = ['design','testcase','development','fix','retest','delivery','closure'];
  const PHASE_LABELS = {design:'设计',testcase:'用例',development:'开发',fix:'修复',retest:'重测',delivery:'交付',closure:'关闭'};
  const PHASE_STATUS = {done:'完成',active:'进行中',blocked:'阻塞',pending:'待开始',in_progress:'进行中',completed:'完成',revision_needed:'需修改'};
  const PHASE_COMMANDS = {design:'/openflow:design',testcase:'/openflow:plan',development:'/openflow:build',delivery:'/openflow:close',closure:'',fix:'/openflow:fix',retest:'/openflow:retest'};
  const PHASE_DESCRIPTIONS = {design:'需求探索与澄清 → 生成提案(proposal) → 编写设计文档(design) → 自检并产出设计报告',testcase:'基于设计文档生成测试方案(testing-guide) → 拆分任务清单(tasks) → 自检并产出用例报告',development:'环境请求与确认 → 编码实现 → 编写AT(Acceptance Tests) → 编译部署 → 执行AT → 一次统一修复 → 产出测试报告',fix:'汇总失败用例 → 统一修复代码 → 重编译替换 → 重跑失败用例验证 → 仍失败则标记失败回滚(最多循环5次)',retest:'列出全部用例 → 选部分/全部重跑(不重编译，用当前已部署代码) → 更新test_results → 产出retest-report',delivery:'更新文档 → 归档需求(changes/specs) → 产出交付报告 → 完结需求生命周期',closure:'需求已完成归档，无需执行操作。如需查看历史报告或重新打开需求，请使用其他命令。'};

  // ---- State ----
  let currentDemand = null;
  let selectedPhase = null;
  let displayedDemandId = null;
  let isExecuting = false;   // true while a phase command is running

  // ---- Event listeners (via delegation) ----
  function setupEventDelegation() {
    // Phase box clicks
    mainContent.addEventListener('click', function(e) {
      const phaseBox = e.target.closest('.phase-box');
      if (phaseBox) {
        e.preventDefault();
        const pid = phaseBox.getAttribute('data-phase');
        if (pid && pid !== selectedPhase && currentDemand) {
          selectedPhase = pid;
      ...
      // Artifact clicks
      const artifactItem = e.target.closest('.phase-artifact-item');
      if (artifactItem) {
        e.preventDefault();
        const path = artifactItem.getAttribute('data-path');
        if (path) api.postMessage({command:'openFile', path: path});
        return;
      }

      // Execute button
      const execBtn = e.target.closest('#executeBtn');
      if (execBtn && currentDemand && selectedPhase && !isExecuting) {
        e.preventDefault();
        isExecuting = true;
        execBtn.disabled = true;
        execBtn.textContent = '执行中...';
        if (nextStepsBanner) nextStepsBanner.classList.add('hidden');
        api.postMessage({command:'runPhase', demandId: currentDemand.id, phase: selectedPhase});
        return;
      }

      // Gate pass button
      const gatePassBtn = e.target.closest('#gatePassBtn');
      if (gatePassBtn && currentDemand && selectedPhase) {
        e.preventDefault();
        gatePassBtn.disabled = true;
        gatePassBtn.textContent = '提交中...';
        api.postMessage({command:'reviewGate', demandId: currentDemand.id, phase: selectedPhase, action:'pass'});
        return;
      }

      // Gate reject button
      const gateRejectBtn = e.target.closest('#gateRejectBtn');
      if (gateRejectBtn && currentDemand && selectedPhase) {
        e.preventDefault();
        gateRejectBtn.disabled = true;
        gateRejectBtn.textContent = '提交中...';
        api.postMessage({command:'reviewGate', demandId: currentDemand.id, phase: selectedPhase, action:'reject'});
        return;
      }
    });

    // Next-steps banner buttons
    if (nextStepsBanner) {
      nextStepsBanner.addEventListener('click', function(e) {
        const btn = e.target.closest('.btn-ns');
        if (btn && currentDemand) {
  ...
  function handlePhaseComplete(msg) {
    ...
    isExecuting = false;
    const phaseLabel = msg.phaseLabel || msg.phase || '';
    const steps = msg.nextSteps || [];
    if (!steps.length) {
      nextStepsBanner.innerHTML = '<div class="ns-title">✓ ' + esc(phaseLabel) + ' 完成 — 需求生命周期结束</div>';
      nextStepsBanner.classList.remove('hidden');
      return;
    }
    const demandId = currentDemand ? currentDemand.id : '';
    let html = '<div class="ns-title">✓ ' + esc(phaseLabel) + ' 阶段执行结束，下一步可执行：</div><div class="ns-list">';
    for (let i = 0; i < steps.length; i++) {
      const s = steps[i];
      html += '<button class="btn-ns" data-phase="' + esc(s.phase) + '">' +
  ...
  function handleState(payload) {
    ...
    // Reset selected phase when the demand advances to a new phase
  ...
  function showDemandDetail(d) {
    ...
    // Re-enable execute button on new state
    isExecuting = false;

    const phases = d.phases || {};
    const isClosure = selectedPhase === 'closure';

    // Build phase grid
    let phaseBoxes = '';
    for (let i = 0; i < PHASE_ORDER.length; i++) {
      const p = PHASE_ORDER[i];
      const pdata = phases[p] || {};
      const status = pdata.status || 'pending';
      const gateStatus = (pdata.gate && pdata.gate.status) || '';
      let cls = 'phase-box';
      ...
      let gateHtml = gateStatus ? '<div class="phase-box-gate ' + gateStatus + '">' + gateIcon(gateStatus) + '</div>' : '';
      let statusIcon = '';
      if (status === 'done' || status === 'completed') statusIcon = ' ✓';
      else if (status === 'blocked') statusIcon = ' ⛔';
      phaseBoxes += '<div class="' + cls + '" data-phase="' + p + '">' +
    ...
    // Build phase info
    const selPhaseData = phases[selectedPhase] || {};
    const selArtifacts = selPhaseData.artifacts || [];
    const selGateStatus = (selPhaseData.gate && selPhaseData.gate.status) || 'unknown';
    const selGatePending = selGateStatus === 'pending';
    const selPhaseStatus = selPhaseData.status || 'pending';
    const selPhaseCmd = PHASE_COMMANDS[selectedPhase] || '';
    const selPhaseDesc = PHASE_DESCRIPTIONS[selectedPhase] || '暂无说明';
    const selPhaseReport = selPhaseData.report || '';
    ...
    const phaseInfoHtml =
      '<div class="phase-info">' +
        ...
        '<div class="phase-info-gate">审核：' + gateLabel(selGateStatus) + '</div>' +
      '</div>';
    ...
    if (selArtifacts.length > 0) {
      selArtHtml = '<div class="phase-artifacts visible">' +
        ...
        '<ul>';
      for (let j = 0; j < selArtifacts.length; j++) {
        const a = selArtifacts[j];
        const label = a.split('/').pop() || a;
        selArtHtml += '<li class="phase-artifact-item" data-path="' + esc(a) + '"><span>📄</span><span>' + esc(label) + '</span></li>';
      }
      selArtHtml += '</ul></div>';
    } else {
      selArtHtml = '<div class="phase-artifacts visible">' +
    ...
    if (isClosure) {
      footerHtml += '<span class="completed-badge">✔ 已完成</span>';
    } else {
      footerHtml += '<button class="btn btn-run" id="executeBtn"' +
        (isExecuting ? ' disabled' : '') + '>' +
        (isExecuting ? '执行中...' : '▶ 执行 ' + esc(PHASE_LABELS[selectedPhase] || selectedPhase)) +
        '</button>';
    }
    if (selGatePending) {
      footerHtml += '<button class="btn-gate" id="gatePassBtn">✓ 审核通过</button>';
      footerHtml += '<button class="btn-gate" id="gateRejectBtn">✗ 打回</button>';
    }
    footerHtml += '<span class="gate-label">' + gateLabel(selGateStatus) + '</span>';
    footerHtml += '</div>';
  ...
  function gateLabel(s) {
    return s === 'passed' ? '✓ 已通过' : s === 'pending' ? '⏱ 待审核' : s === 'rejected' ? '✗ 已打回' : '无需审核';
  }
...
exports.FlowMasterDashboardProvider = FlowMasterDashboardProvider;
